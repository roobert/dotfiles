...
function _maybeworkon() {
    ...
    # local DEFAULT_MESSAGE_FORMAT="Switching %venv_type: ${BOLD}%venv_name${NORMAL} ${GREEN}[%py_version]${NORMAL}"
    if [[ "$LANG" != *".UTF-8" ]]; then
    ...
    if [[ -z "$VIRTUAL_ENV" || "$venv_name" != "$(_get_venv_name $VIRTUAL_ENV $venv_type)" ]]; then
        ...
        if [[ ! -d "$venv_dir" ]]; then
            printf "Unable to find ${BOLD}$venv_name${NORMAL} virtualenv\n"
            printf "If the issue persists run ${BOLD}rmvenv && mkvenv${NORMAL} in this directory\n"
            return
        fi
...
function check_venv()
{
    ...
    if [[ -n "$venv_path" ]]; then
        ...
        if [[ -f "$venv_path" ]] && [[ "$file_owner" != "$(id -u)" ]]; then
            ...
            printf "Change ownership of ${BOLD}$venv_path${NORMAL} to ${BOLD}'$USER'${NORMAL} to fix this\n"
            ...
            printf "Run the following command to fix this: ${BOLD}\"chmod 600 $venv_path\"${NORMAL}\n"
        else
    ...
    if [[ "$venv_type" != "unknown" ]]; then
        printf "Python ${BOLD}$venv_type${NORMAL} project detected. "
        printf "Run ${BOLD}mkvenv${NORMAL} to setup autoswitching\n"
    fi
    _default_venv
}
...
function _default_venv()
{
    ...
    if [[ -n "$AUTOSWITCH_DEFAULTENV" ]]; then
        ...
        # _autoswitch_message "Deactivating: ${BOLD}${BOLD}%s${NORMAL}\n" "$venv_name"
        deactivate
    fi
}
...
function mkvenv()
{
    ...
    if [[ "$venv_type" == "pipenv" ]]; then
        ...
        if [[ -f "$AUTOSWITCH_FILE" ]]; then
            ...
            printf "Creating ${BOLD}%s${NONE} virtualenv\n" "$venv_name"
...
function install_requirements() {
    if [[ -f "$AUTOSWITCH_DEFAULT_REQUIREMENTS" ]]; then
        printf "Install default requirements? (${BOLD}$AUTOSWITCH_DEFAULT_REQUIREMENTS${NORMAL}) [y/N]: "
        read ans
    ...
    if [[ -f "$PWD/setup.py" ]]; then
        printf "Found a ${BOLD}setup.py${NORMAL} file. Install dependencies? [y/N]: "
        read ans
    ...
    do
        printf "Found a ${BOLD}%s${NORMAL} file. Install? [y/N]: " "$requirements"
        read ans
